...
$rules = Rules::orderBy("created_at","desc")->get();
?>
@if($rules->count() == 0)
<h2>No Rules Defined Yet</h2>
@else
<ul class="list-group">
...
<script>
    $(document).ready(function (){
        $(".editrule").click(function(){
            ...
            var url= "<?php echo url("rules/edit/") ?>/";
            url += id;
        ...
        $(".deleterule").click(function(){
            var id = $(this).parent().attr("id");
            var url= "<?php echo url("rules/delete") ?>/";
            url += id;
            $(".deleterule").show("slow").parent().parent().find("span").remove();
            var btn = $(this).parent().parent();
            $(this).hide("slow").parent().append("<span><br><b>Are You Sure you want to delete this rule</b><br /> <a href='#s' id='yes' class='btn btn-success btn-xs'><i class='fa fa-check'></i> Yes</a> <a href='#s' id='no' class='btn btn-danger btn-xs'> <i class='fa fa-times'></i> No</a></span>");
            $("#no").click(function(){
